...
OpenClaw 应先把它整理成临时 patrol request，再交给脚本执行。
...
4. 用户说“可能是厨房/后厨/室外/配电间/仓库”等，不要直接当事实；写入 `hints.location_hint` 或 `hints.user_label`。
...
第三路：hints.location_hint=用户说可能是厨房
第十一路：hints.location_hint=用户说可能是配电间
